...
def train_and_save_sleep_xgb_model(dataset_path):
    ...
    features = scaler.fit_transform(dataset[cat_cols])
    dump(scaler, './xgb_sleep_scaler.joblib')
    
    label_encoder_y = LabelEncoder()
    target_encoded = label_encoder_y.fit_transform(dataset['Sleep Disorder'])

    model = xgb.XGBClassifier(objective='multi:softmax', num_class=len(set(target_encoded)))
...
def predict_sleep_disorders(new_data, model, label_encoders, scaler, label_y_encoder):
    features = []
    for col in label_encoders:
        # Transform each feature using the corresponding label encoder
    ...
    prediction = label_y_encoder.inverse_transform(prediction)
    return prediction
...
if model_type == "Skin Health Prediction":
    ...
    st.write("Your Sleep Disorder Prediction Results:")
    st.write(prediction)
    st.write("You are not likely having any sleeping disorder! 😋")
