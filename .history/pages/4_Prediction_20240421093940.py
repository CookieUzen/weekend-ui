...
# Function to train and save the XGBoost model for sleep disorder predictions
def train_and_save_sleep_xgb_model(dataset_path):
    ...
    for col in cat_cols:
        ...
        dump(label_encoders[col], f'{col}_label_encoder_xgb.joblib')
    ...
    model = xgb.XGBClassifier(objective='multi:softprob', num_class=4)  # Assuming 4 classes for the disorders
    model.fit(features_scaled, target)
    dump(model, 'xgb_sleep_model.joblib')
...
def predict_sleep_disorders(new_data, label_encoders, scaler, model):
    cat_cols = ['Gender', 'Age', 'Sleep Duration', 'BMI Category', 'Blood Pressure', 'Heart Rate', 'Daily Steps']
    for col in cat_cols:
...
if model_type == "Skin Health Prediction":
    ...
    with st.status("Loading", expanded=True, state="running") as status:
        ...
        if model:
            ...
            model, encoders, scaler = train_and_save_sleep_xgb_model('data/Sleep_health_and_lifestyle_dataset.csv')
        
        for percent_complete in range(100):
            time.sleep(0.01)
            # increment the progress bar
            my_bar.progress(percent_complete + 1, text=progress_text)
    prediction = predict_sleep_disorders(user_data, model, label_encoders, scaler)
    st.write("Your Sleep Disorder Prediction Results:")
    st.write(prediction)
