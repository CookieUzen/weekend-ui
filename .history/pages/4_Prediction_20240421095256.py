...
def train_and_save_sleep_xgb_model(dataset_path):
    ...
    #num_cols = ['BMI Category']  # Assuming 'BMI' is a numeric column already properly scaled; adjust if not applicable.
    ...
    dump(label_encoder_y, './sleep_disorder_label_encoder_xgb.joblib')
...
def predict_sleep_disorders(new_data, model, label_encoders, scaler, label_encoder_y):
    ...
    for col in label_encoders.keys():
        if col in new_data:
            # Transform and append each column's data using the appropriate encoder
            encoded_feature = label_encoders[col].transform([new_data[col]])[0]
            features.append(encoded_feature)
    
    # Scale the features using the scaler
    features_scaled = scaler.transform([features])  # Ensure features are in the shape (1, -1)

    # Predict using the model
    prediction = model.predict(features_scaled)
    decoded_prediction = label_encoder_y.inverse_transform(prediction)  # Decode numeric prediction back to string label
